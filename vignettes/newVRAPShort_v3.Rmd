...
clean <- TRUE # clean up the files made by VRAP 1.0
...
inputs$BufferNumSteps = round((inputs$BufferEnd - inputs$BufferStart) / inputs$BufferStepSize + 1) # number of ER targets

```

Then we use the code above to create the function to run the simulations for each target ER. This allows us to iterate over a range of exploitation rates.  The function is `RunSims2R()` in the `VRAPS` package.  To view the function type:
...
Now we can assemble this code into an R function that iterates through the different exploitation rates and calls the C++ function.  This function is `RunSims2C()` in the `VRAPS` package.
