...
inputs <- list(
  ...
  BufferStart = 0,
  BufferEnd = 0.8,
  BufferStepSize = 0.02, # stepsize in terms of the exploitation rate, baseER*stepSize  
    
  NYears = 25, # years in each forward projectiong
  NRuns = 1000 # the number of times the simulation should be repeated.
)
inputs$BufferNumSteps = round((inputs$BufferEnd - inputs$BufferStart) / inputs$BufferStepSize + 1) # number of steps
...
```{r}
RunSims2R <- function(input,rngSeed=NULL,verbose=FALSE){
  # initialize output array
  totEsc <- array(NA,dim=c(inputs$BufferNumSteps+1,inputs$NRuns,inputs$NYears))
  HRscale <- 1 # multiply this times the harvest rates.
  ...
  for(ERind in 1:(inputs$BufferNumSteps+1)){
...
```{r, echo=FALSE}
source(here("R/generateRavFile.R"))
print(generateRavFile)
```
...
plotSimAvg <- function(simDat,vrapOut){
  ...
  for(i in 1:(inputs$BufferNumSteps+1)){
...
NumericMatrix simFish(int NRuns, int NYears, 
  ...
  for(int sim = 0; sim < NRuns; sim++) { // loop through simulations
    Cohort = clone(CohortStart); // initialize population. Use clone to create a deep copy (i.e don't just copy a pointer)
    logSRerror = rnorm(1, 0, SRErrorA)[0];
    for(int year = 0; year < NYears; year++) { // loop through years
      Cohort = Cohort*(1-NatMort);
      if(MgmtError) ER = std::min(targetER * rgamma(1, GammaMgmtA, GammaMgmtB)[0],1.0); else ER = targetER;
      //Rcpp::Rcout << ER << ",";
      if(ER==0){
...
# run the simulation
xx <- simFish(NRuns=1000,NYears=25,targetER=0.2,
...
```{r}
RunSims2C <- function(input,rngSeed=NULL){
  # initialize output array
  totEsc <- array(NA,dim=c(inputs$BufferNumSteps+1,inputs$NRuns,inputs$NYears))
  HRscale <- 1 # multiply this times the harvest rates.
  # calcualte the target exploitation rates based on start, step size, and steps
  ...
  for(ERind in 1:(inputs$BufferNumSteps+1)){
    print(paste("============= target ER =",targetER[ERind]))
    totEsc[ERind,,] <- simFish(NRuns=inputs$NRuns,NYears=inputs$NYears,targetER=targetER[ERind],
              MgmtError=inputs$MgmtError, 
              GammaMgmtA=inputs$GammaMgmtA,GammaMgmtB=inputs$GammaMgmtB,
              errorType=inputs$errorType,
              SRErrorA=inputs$SRErrorA,SRErrorB=inputs$SRErrorB,
...
plot(1,1,xlim=c(0,0.8),ylim=c(0,1),xlab="Year",ylab="% years below critical Esc threshold",type="n",bty="l")
targetER <- inputs2$BufferStart + inputs2$BufferStepSize * (0:inputs2$BufferNumSteps)
